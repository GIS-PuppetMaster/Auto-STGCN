...
def weight_matrix(file_path, num_of_vertices, sigma2=0.1, epsilon=0.5, scaling=True, file_type='mat', id_file_name=None):
    '''
    Load weight matrix
    ...
    file_type: string, 'mat' or 'list', 'mat' for original STGCN data set PeMSD7, 'list' for PEMS03/04/07/08
    ...
    if file_type=='mat':
        ...
        adj = get_adjacency_matrix(file_path, num_of_vertices, id_filename=id_file_name)
    else:
        raise ValueError("file_type error, must be "
                         "mat or list!")
